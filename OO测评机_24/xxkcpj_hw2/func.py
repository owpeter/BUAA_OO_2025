...
def generate_simple_expression(variable):
    a = random.randint(1, 3)  # 随机整数系数
    b = random.randint(1, 3)  # 随机幂次
...
def generate_complex_expression(variable):
    ...
    while True:
        ...
        # 生成随机三角表达式
        expr1 = sympy.sin((variable + random.randint(0, 1))) ** random.randint(1, 2)
        expr2 = sympy.cos((variable - random.randint(0, 1))) ** random.randint(1, 2)

        # 组合成复杂表达式
        complex_expression = a * (variable ** b) * expr1 * expr2

        if len(str(complex_expression)) > 30:
            continue
        else:
...
# 定义递推函数
def recursive_function(n, x_expr, y_expr, a, b):
    """
    实现递推关系：
...
# 嵌套函数调用生成器
def generate_nested_expression(n, max_depth=1):
    """
    ...
    if random.choice([True, False]):
        ...
        a = random.randint(1,5)
        b = random.randint(1,5)
        return recursive_function(nested_n, generate_nested_expression(nested_n, max_depth - 1), generate_nested_expression(nested_n, max_depth - 1), a, b)
    else:
        # 返回普通复杂表达式
        return generate_complex_expression(x)
...
def genFunc():
    ...
    def_f0 = 'f{0}(x,y) = ' + f"{f0}".replace("**", "^")
    def_f1 = 'f{1}(x,y) = ' + f"{f1}".replace("**", "^")
    def_fn = 'f{n}(x,y) = 'f"{a} * f(n-1)({g}, {h}) + {b} * f(n-2)({g_prime}, {h_prime}) + {i}".replace("**", "^")
    ...
    ultilize = 'f{'+f'{n}'+'}'+f'({x_expr}, {y_expr})'
    # print(ultilize)
    ...
    # 计算递推函数
    result = recursive_function(n, x_expr, y_expr, a, b)


    return def_f0, def_f1, def_fn, ultilize, result
    # print(result)
    # print(f"f_{n}({x_expr}, {y_expr}) = {sympy.expand(result)}")


    # f = sympy.sympify(result)
    # sympy_result = sympy.expand(f)
    # ans = str(sympy_result).replace("**", "^")
    # print(f"f_{n}({x_expr}, {y_expr}) = {ans}")
