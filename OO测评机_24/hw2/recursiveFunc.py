...
def generate_complex_expression(variable):
    ...
    """
    # 随机生成系数和幂次
    a = random.randint(1, 5)  # 随机整数系数
    b = random.randint(0, 3)  # 随机幂次

    # 生成随机三角表达式
    expr1 = sp.sin(variable + random.randint(1, 3)) ** random.randint(1, 2)
    expr2 = sp.cos(variable - random.randint(1, 3)) ** random.randint(1, 2)
...
print(f"f0 = {f0}")
print(f"f1 = {f1}")
print(f"fn = {a} * f(n-1)(g(x, y)={g}, h(x, y)={h}) + {b} * f(n-2)(g'(x, y)={g_prime}, h'(x, y)={h_prime}) + i(x, y)={i}")
...
print(f"f_{n}({x_expr}, {y_expr}) = {sp.simplify(result)}")