...
{
        for (int i = 0; i < ELEVATOR_CNT + 1; i++) {
                elevators[i].floor = 5;
        }
        String inputName;
        String outputName;
...
int parseNextFloorInt(String *src);
...
Person parsePerson(String src)
{
        ...
        res.start = parseNextFloorInt(&src);
        res.dest = parseNextFloorInt(&src);
        res.elevatorId = parseNextInt(&src);
        // printf("%d-%d->%d::%d",res.personId, res.start, res.dest, res.elevatorId);
        return res;
}
...
int parseNextFloorInt(String *src)
{
        int res = 0;
        int flag = 0;
        while (!((**src == 'B' || **src == 'F') && *(*src + 1) >= '0' && *(*src + 1) <= '9'))
        {
                *src += 1;
        }
        if (**src == 'F') flag = 1;
